...
public class GlamourerFunctions : IDisposable
{
    ...
    GagSpeakGlamourEvent gagSpeakGlamourEvent, OnFrameworkService onFrameworkService, HardcoreManager hardcoreManager) {
        ...
        _logger.LogInformation($"[GlamourerService]: GlamourerFunctions initialized!");
    }

    public void Dispose() {
        _logger.LogInformation($"[GlamourerService]: Disposing of GlamourerService");
    ...
    public void FrameworkUpdate(IFramework framework) {
        ...
        if(finishedDrawingGlamChange){
            ...
            if(disableGlamChangeEvent) {
                ...
                _logger.LogInformation($"[FrameworkUpdate] Re-Allowing Glamour Change Event");
            }
        } 
    }
    ...
    private void GlamourerChanged(nint address, StateChangeType type) {
        ...
        {
            _logger.LogVerbose($"[GlamourerChanged]: Change not from Character, IGNORING");
            return;
        }
        ...
        {
            _logger.LogVerbose($"[GlamourerChanged]: Wardrobe is disabled, so we wont be updating/applying any gag items");
            return;
        }
        ...
        {
            _logger.LogVerbose($"[GlamourerChanged]: Blocked due to request variables");
            return;
        }
        ...
        {
            _logger.LogVerbose($"[GlamourerChanged]: StateChangeType is {(StateChangeType)type}");
            _gagSpeakGlamourEvent.Invoke(UpdateType.RefreshAll);
        }
        else // it is not a type we care about, so ignore
        {
            _logger.LogVerbose($"[GlamourerChanged]: GlamourerChanged event was not a type we care about, so skipping (Type was: {(StateChangeType)type})");
        }
    }
    ...
    public async void GlamourEventFired(object sender, GagSpeakGlamourEventArgs e) {
        ...
        if(_characterHandler.playerChar._enableWardrobe) {
            _logger.LogInformation(" "+ e.UpdateType.ToString().ToUpper()+" GLAMOUR EVENT FIRED ");
            // conditionals:
            try{
                ...
                {
                    _logger.LogDebug($"[GlamourEventFired]: Processing Restraint Set Update");
                ...
                if(e.UpdateType == UpdateType.DisableRestraintSet && _characterHandler.playerChar._allowRestraintSetAutoEquip)
                {
                    try{ 
                        ...
                        _logger.LogError($"Error reverting glamourer to automation");
                    }
                    // now reapply the gags
                    if(_characterHandler.playerChar._allowItemAutoEquip) {
                        _logger.LogDebug($"[GlamourEventFired]: Reapplying gags");
                        await ApplyGagItemsToCachedCharacterData();
                        _logger.LogDebug($"[GlamourEventFired]: Reapplying blindfold");
                        if( _hardcoreManager.IsBlindfoldedForAny(out int enabledIdx, out string playerWhoBlindfoldedYou)) {
                            await EquipBlindfold(enabledIdx);
                        } else {
            ...
            _logger.LogDebug($"[GlamourEventFired]: Wardrobe is disabled, so we wont be updating/applying any gag items");
        }
    }
