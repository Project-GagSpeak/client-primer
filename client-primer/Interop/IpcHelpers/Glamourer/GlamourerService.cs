...
public sealed class GlamourerService : IDisposable
{
    ...
    private bool CheckGlamourerApiInternal() {
        ...
        try {
            ...
            if (!apiAvailable) {
                _logger.LogError($"[GlamourerService]: Glamourer inactive. All Wardrobe functionality will not work.");
            }
        }
    }
        ...
        try {
            await _OnFrameworkService.RunOnFrameworkThread(() => {
                ...
                if (gameObj is Character c) {
                    _logger.LogVerbose("[ApplyAllAsyncIntrop] Calling on IPC: GlamourerApplyAll");
                    _ApplyOnlyEquipment!.Invoke(customization, c.ObjectIndex, 0, Glamourer.Api.Enums.ApplyFlag.Equipment);
                }
            }).ConfigureAwait(false);
        } catch (Exception) {
            _logger.LogDebug("[ApplyAllAsyncIntrop] Failed to apply Glamourer data");
        } 
    }
    */
    ...
    public async Task<string> GetCharacterCustomizationAsync(IntPtr character) {
        ...
        try {
            ...
            return await _OnFrameworkService.RunOnFrameworkThread(() => {
                ...
                // if the game object is the character, then get the customization for it.
                if (gameObj is Character c) {
                    _logger.LogVerbose("[GetCharacterCustomizationAsync] Calling on IPC: GlamourerGetAllCustomizationFromCharacter");
    ...
    */
    ...
    public async Task SetItemToCharacterAsync(IntPtr character, Glamourer.Api.Enums.ApiEquipSlot slot, ulong item, byte dye, uint variant) {
        ...
        try {
            ...
            // if at any point this errors, return an empty string as well.
            _logger.LogWarning($"[SetItemOnceToCharacterAsync] Failed to set item to character with slot {slot}, item {item}, dye {dye}, and key {variant}, {ex}");
            return;
        }
    }
        ...
        try
        {
            ...
            await _OnFrameworkService.RunOnFrameworkThread(async () => {
                try
                {
                    ...
                    if (gameObj is Character c)
                    {
                        _logger.LogVerbose("[GlamourRevertIPC] Calling on IPC: GlamourerRevertToAutomationCharacter");
                        GlamourerApiEc result = _RevertToAutomation.Invoke(c.ObjectIndex);
                        _logger.LogVerbose($"[GlamourRevertIPC] Revert to automation result: {result}");
                        // if it doesnt return success, revert to game instead
                        if(result != GlamourerApiEc.Success)
                        {
                            _logger.LogWarning($"[GlamourRevertIPC] Revert to automation failed, reverting to game instead");
                ...
                {
                    _logger.LogWarning($"[GlamourRevertIPC] Error during GlamourerRevert: {ex}");
                }
            }).ConfigureAwait(false);
        }
        catch (Exception ex)
