...
namespace UpdateMonitoring.Chat.Handler;
...
public class PuppeteerHandler
{
    ...
    public PuppeteerHandler(CharacterHandler characterHandler, IDataManager dataManager)
    {
    ...
    public bool ContainsGlobalTriggerWord(string messageRecieved, out string puppeteerMessageToSend)
    {
        ...
        if (match.Success)
        {
            var remainingMessage = messageRecieved.Substring(match.Index + match.Length).Trim();
    ...
    public bool ContainsTriggerWord(string SenderName, string messageRecieved, out string puppeteerMessageToSend)
    {
        ...
        if (AltCharHelpers.IsPlayerInWhitelist(SenderName, out int whitelistCharIdx))
        {
            ...
            var triggerWordArray = triggerWords.Split('|');

            foreach (var triggerWord in triggerWordArray)
            {
                if (string.IsNullOrEmpty(triggerWord) || string.IsNullOrWhiteSpace(triggerWord) || triggerWord == "" || triggerWord == " ")
                {
                    continue;
    ...
    {
        var startIndex = str.IndexOf(_characterHandler.playerChar._uniquePlayerPerms[indexOfWhitelistedChar]._StartCharForPuppeteerTrigger);
        var endIndex = str.IndexOf(_characterHandler.playerChar._uniquePlayerPerms[indexOfWhitelistedChar]._EndCharForPuppeteerTrigger);
