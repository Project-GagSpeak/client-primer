...
public class CompactUi : WindowMediatorSubscriberBase
{
    ...
    private void UpdateShouldOpenStatus(Pair? specificPair = null, StickyWindowType type = StickyWindowType.None)
    {
        ...
        if (specificPair != null)
        {
            ...
            if (indexToKeep != -1)
            {
                ...
                var currentStatus = _allUserPairDrawsDistinct[indexToKeep].Pair.ShouldOpenPermWindow;
        ...
        for (var i = 0; i < _allUserPairDrawsDistinct.Count; i++)
        {
            if (i != indexToKeep)
            {
    ...
    protected override void DrawInternal()
    {
        ...
        if (_apiController.ServerState is ServerState.Connected)
        {
            ...
            // grab the yposition
            var topMenuEnd = ImGui.GetCursorPosY();
