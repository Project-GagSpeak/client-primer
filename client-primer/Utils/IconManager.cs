...
public class IconManager : IDisposable
{
    ...
    public class GraphicFont : IDisposable {
        ...
        private static ISharedImmediateTexture? _textureWrap;
            ...
            public void Draw(Vector2? drawSize = null, bool highQuality = true) {
                ...
                if (_textureWrap == null) {
                    if (_gameConfig.TryGet(SystemConfigOption.PadSelectButtonIcon, out _padIconMode) && _padIconMode < _texturePath.Length) {
                        _textureWrap = _textureProvider.GetFromGame(_texturePath[_padIconMode]);
                    }
                    ImGui.Dummy(drawSize ?? size);
                    return;
                }

                var textureSize = new Vector2(_textureWrap.GetWrapOrEmpty().Width, _textureWrap.GetWrapOrEmpty().Height);
                ...
                ImGui.Image(_textureWrap.GetWrapOrEmpty().ImGuiHandle, drawSize ?? size, uv0, uv1);
            }
        }
    ...
    public void Dispose() {
        disposed = true;
        var c = 0;
        _logger.LogDebug("Disposing icon textures");
        foreach (var texture in iconTextures.Values.Where(texture => texture != null)) {
            c++;
            texture.Dispose();
        }

        _logger.LogDebug($"Disposed {c} icon textures.");
        iconTextures.Clear();
    ...
    private void LoadIconTexture(int iconId, bool hq = false) {
        Task.Run(() => {
            try {
                ...
                _logger.LogError($"Failed loading texture for icon {iconId} - {ex.Message}");
            }
        });
    }
